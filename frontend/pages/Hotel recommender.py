...
with result_col:
    ...
    if submitted:
        ...
        # --- NORMALIZE BACKEND KEYS TO MATCH FRONTEND UI ---
        for h in hotels:
            # Fix Name 
            if "name" in h and "hotel_name" not in h:
                h["hotel_name"] = h["name"]
            
            # Fix Price
            if "avg_price_per_night" in h and "price_per_night" not in h:
                h["price_per_night"] = float(h["avg_price_per_night"])
                
            # Fix Rating (Backend gives 10.0 scale, UI wants 5.0 scale)
            if "review_score" in h and "rating" not in h:
                h["rating"] = round(float(h["review_score"]) / 2, 1)
            elif "star_rating" in h and "rating" not in h:
                h["rating"] = float(h["star_rating"])
                
            # Fix Match Score (Backend gives 0-100, UI progress bar requires 0.0 - 1.0)
            if "match_score" in h and h["match_score"] > 1.0:
                h["match_score"] = float(h["match_score"]) / 100.0
                
            # Make sure INR price exists
            if "avg_price_per_night_inr" in h and "price_per_night_inr" not in h:
                h["price_per_night_inr"] = float(h["avg_price_per_night_inr"])
            elif "price_per_night_inr" not in h and "price_per_night" in h:
                h["price_per_night_inr"] = to_inr(h["price_per_night"])
        # --------------------------------------------------------

        if hotels:
            st.success(f"🏨 Found **{len(hotels)}** hotels")
            for i, hotel in enumerate(hotels, 1):
                # Now safely using normalized keys
                price_inr = hotel.get("price_per_night_inr", to_inr(hotel.get("price_per_night", 0)))
                with st.expander(
                    ...
                    st.progress(min(1.0, max(0.0, float(hotel["match_score"]))))
